...
import { useRef, useCallback, useState, useReducer, useMemo } from 'react';
...
function createUseRefetch(client, { defaults: { retry: defaultRetry } }) {
  ...
  const useRefetch = function useRefetch2(refetchOptions) {
    ...
    const [reducerState, dispatch] = useReducer(UseRefetchReducer, void 0, InitUseRefetchReducer);
    ...
    const refetchCallback = useCallback(async (refetchArg) => {
      ...
      const selectionsToRefetch = Array.from(selections).filter((v) => v.type === SelectionType.Query);
    ...
    const state = useMemo(() => Object.assign(reducerState, {
      startWatching,
      stopWatching

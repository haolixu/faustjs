...
function createUseTransactionQuery(client, {
  ...
  const useTransactionQuery = function useTransactionQuery2(fn, ...[queryOptions]) {
    ...
    const [state, dispatchReducer] = useReducer(UseTransactionQueryReducer, opts, InitUseTransactionQueryReducer);
    ...
    const queryCallback = useCallback((resolveOptsArg = {}, fetchPolicyArg = fetchPolicy, cacheChangeCall) => {
      if (skip) {
    ...
    const queryCallbackWithPromise = useCallback((inlineCall) => {
      var _a2;
      if (skip)
        return;

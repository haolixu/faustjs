...
const defaultFilePath = path.resolve(process.cwd(), "gqty.config.cjs");
  ...
  try {
    if (process.env.NODE_ENV === "test") {
      ...
      const cjsLoader = (filePath) => {
        const requireConfig = module$1.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('config.js', document.baseURI).href)));
      ...
      if (!config || config.isEmpty) {
        ...
        if (NODE_ENV !== "test" && NODE_ENV !== "production" && innerState.__innerState.isCLI) {
          const { format } = (await Promise.resolve().then(function () { return require('./prettier.js'); })).formatPrettier({
            parser: "typescript"
          });
...
exports.DUMMY_ENDPOINT = DUMMY_ENDPOINT;
exports.defaultConfig = defaultConfig;
exports.getValidConfig = getValidConfig;
exports.gqtyConfigPromise = gqtyConfigPromise;

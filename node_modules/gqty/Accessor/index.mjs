...
import { SchemaUnionsKey, parseSchemaType } from '../Schema/types.mjs';
import { SelectionType, Selection } from '../Selection/selection.mjs';
import { isInteger } from '../Utils/index.mjs';
import { decycle, retrocycle } from '../Utils/cycle.mjs';
import { isObject, isObjectWithType } from '../Utils/object.mjs';
...
function createSchemaUnions(schema) {
  ...
  const unions = Object.entries(schema[SchemaUnionsKey] || {}).reduce((acum, [name, unionTypes]) => {
...
function createAccessorCreators(innerState) {
  ...
  function extractDataFromProxy(value) {
    if (accessorCache.isProxy(value)) {
      ...
      return retrocycle(JSON.parse(JSON.stringify(value)));
    }
    return value;
  }
      ...
      return new Proxy(proxyValue, {
        ...
        get(target, key, receiver) {
          if (key === "length") {
            ...
            return () => decycle(innerState.clientCache.getCacheFromSelection(prevSelection, []));
          }
          let index;
          try {
            index = parseInt(key);
  ...
  function getTypename(selection) {
    ...
    if (isObjectWithType(cacheValue))
        ...
        get(target, key, receiver) {
          ...
          if (schemaValue instanceof SchemaUnion) {
            ...
            const resolve = (args) => {
              ...
              throw new GQtyError(`GraphQL Type not found: ${pureType}, available fields: "${Object.keys(schemaValue).join(" | ")}"`);
            };
            if (__args) {
              const resolveInfo = {
                key,
...
export { SchemaUnion, createAccessorCreators, createSchemaUnions };

...
function createUseMetaState(client) {
  ...
  const useMetaState = function useMetaState2(opts = defaultEmptyOpts) {
    ...
    } = useBuildSelections(opts.filterSelections, buildSelection, getProxySelection, useMetaState2);
    ...
    const getState = useCallback((isMounted = isMountedRef) => {
      let isFetching;
      if (scheduler.pendingSelectionsGroups.size) {
        if (hasFilterSelections) {
          isFetching = isAnySelectionIncludedInMatrix(selectionsToFilter, scheduler.pendingSelectionsGroups);
        } else {
          isFetching = true;
        }
    ...
    const setStateIfChanged = useCallback(function setStateIfChanged2(isMounted) {
